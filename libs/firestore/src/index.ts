...
import { Firestore, FieldValue, type Settings, type WhereFilterOp } from '@google-cloud/firestore';
...
export async function deleteDoc(
  ...
  assertDatabaseMatch(path.database);
  const db = getFirestore();
  await db.doc(`${path.collection}/${docId}`).delete();
}

export async function queryCollection<T = unknown>(
  path: CollectionPath,
  opts: {
    field: string;
    op: WhereFilterOp;
  ...
  assertDatabaseMatch(path.database);
  const db = getFirestore();
  let q = db.collection(path.collection).where(opts.field, opts.op, opts.value);
  if (opts.orderBy) {
