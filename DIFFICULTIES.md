# Difficulties Encountered: GCP Pastebin Build

A working log of the non-obvious failures, surprises, and dead ends hit while porting the AWS `pastebin-app` to GCP using the `gcp-terraform-cloud-run` skill. Intended as a reference for future projects.
...
2. `pastebingcp-app-bff-runtime` lacked `roles/pubsub.publisher` in the live
