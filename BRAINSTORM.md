# Pastebin on GCP — Brainstorm & Design Plan

> Companion brainstorm to the AWS reference build at
> `/opt/data/serverless/pastebin-app/`. Same product, same
...
**Goals**
- Re-implement the AWS Pastebin on GCP without losing its shape:
...
|------------------|-----------------------------|-------------------------------------------------------------------|-------------------------------------------------------|
| `event-hub`      | `pastebin-event-hub`   | Pub/Sub topics + DLQs + IAM for cross-service publish/subscribe   | Topic `pastebin-events` + DLQ `…-events-dlq`      |
| `app-bff`        | `pastebin-app-bff`     | `POST /shorten`, `GET /me/urls` (auth)                             | Firestore DB `app-db` with `mappings` collection      |
| `redirect-bff`   | `pastebin-redirect-bff`| `GET /{code}` → 302; reads lean view; emits `click.recorded`       | Firestore DB `redirect-db` with `lean_view` (`code`, `longUrl`, `ownerUid`, `createdAt`) |
| `analytics-bff`  | `pastebin-analytics-bff` | `GET /analytics/{code}` (auth, owner-only); aggregates clicks   | Firestore DB `analytics-db` with `clicks` (`count`, `ownerUid`, `lastClickedAt`) |
...
Firestore native trigger (via Eventarc)
  ...
  → decode fields; publish to Pub/Sub topic `pastebin-events`:
      {
...
I will:
1. Initialize the new repo at `/opt/data/serverless/pastebin-app-gcp/`
   and commit the scaffold.
2. Mirror it to a new GitHub repo (`pastebin-app-gcp.git`)
